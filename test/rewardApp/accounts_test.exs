defmodule RewardApp.AccountsTest do
  ...
  describe "users" do
    ...
    test "create_user/1 with valid data creates a user" do
      valid_attrs = %{name: "some name", role: "some role", total_points: 42}

      assert {:ok, %User{} = user} = Accounts.create_user(valid_attrs)
      assert user.name == "some name"
      assert user.role == "some role"
      assert user.total_points == 42
    end
    ...
    test "update_user/2 with valid data updates the user" do
      user = user_fixture()
      update_attrs = %{name: "some updated name", role: "some updated role", total_points: 43}

      assert {:ok, %User{} = user} = Accounts.update_user(user, update_attrs)
      assert user.name == "some updated name"
      assert user.role == "some updated role"
      assert user.total_points == 43
    end
